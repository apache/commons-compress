...
public class ZipArchiveInputStream extends ArchiveInputStream<ZipArchiveEntry> implements InputStreamStatistics {
    ...
    private void drainCurrentEntryData() throws IOException {
        ...
        while (remaining > 0) {
            final long n = super.read(buf.array(), 0, (int) Math.min(buf.capacity(), remaining));
            if (n < 0) {
    ...
    private void readStoredEntry() throws IOException {
        ...
        while (!done) {
            final int r = super.read(buf.array(), off, ZipArchiveOutputStream.BUFFER_SIZE - off);
            if (r <= 0) {
    ...
    private void realSkip(final long value) throws IOException {
        if (value >= 0) {
            ...
            while (skipped < value) {
                ...
                final int x = super.read(skipBuf, 0, (int) (skipBuf.length > rem ? rem : skipBuf.length));
                if (x == -1) {
                    return;
                }
                count(x);
