 ...
 *
 * <p>
 * All methods with a _FILTER suffix are used as preprocessors with the goal of creating a better compression ratio with the compressor that comes next in the
 * chain of methods. 7z will in general only allow them to be used together with a "real" compression method but Commons Compress doesn't enforce this.
 * </p>
 *
 * <p>
 * The BCJ_ filters work on executable files for the given platform and convert relative addresses to absolute addresses in CALL instructions. This means they
 * are only useful when applied to executables of the chosen platform.
 * </p>
 */
