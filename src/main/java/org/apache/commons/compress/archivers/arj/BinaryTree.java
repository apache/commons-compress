...
/**
 * Binary tree of positive values. Copied from org.apache.commons.compress.archivers.lha.BinaryTree and modified for ARJ.
 */
class BinaryTree {
    ...
    /**
     * Constructs a binary tree from the given array that contains the depth (code length) in the binary tree as values in the array and the index into the
     * array as the value of the leaf node. If the array contains a single value, this is a special case where there is only one node in the tree (the root
     * node) and it contains the value. For this case, the array contains the value of the root node instead of the depth in the tree. This special case also
     * means that no bits will be read from the bit stream when the read method is called, as there are no children to traverse.
     *
    ...
    BinaryTree(final int... array) throws CompressorException {
        ...
        final int maxDepth = Arrays.stream(array).max().orElseThrow(() -> new CompressorException("Tree contains no leaf nodes"));
        if (maxDepth == 0) {
